-- Checkout applies promocodes for signed-in customers; signed-in orders
-- attach to the auth-linked customer so My Orders always sees them.

drop function if exists public.storefront_checkout(jsonb, text, text, text, text, text, text);
...
begin
  ...
  -- Promocode (signed-in customers; all rules enforced server-side).
  if nullif(trim(coalesce(p_promocode, '')), '') is not null then
    ...
    select * into v_promo from public.promocodes where code = lower(trim(p_promocode));
  ...
  -- Prefer the auth-linked customer when signed in; else by phone.
  ...
  if v_promo is not null then
    ...
    values (v_promo.id, v_customer, v_order_id, extract(year from current_date)::integer);
  end if;
...
revoke all on function public.storefront_checkout(jsonb, text, text, text, text, text, text, text) from public;
grant execute on function public.storefront_checkout(jsonb, text, text, text, text, text, text, text) to anon, authenticated;
